...
node（节点）：这个概念跟内存控制器（MC）有关，如果系统中的CPU很多，为了简化内存资源的分配以及提高利用效率，CPU设计会有多个内存控制器，共用相同内存控制器的CPU组为一个node。分为UMA和NUMA架构，前者只有一个节点，只有一个内存控制器，后者将分为多个节点，每个节点有一个内存控制器。（为了降低认知负载，我们就只讨论UMA架构的）

zone（区）：节点之下便是zone，zone代表的是内存的用途，把内存划分区域管理，不同用途的内存页属于不同的zone。对应结构体[zone](https://elixir.bootlin.com/linux/v6.6/source/include/linux/mmzone.h#L810)

page（页）：物理页，对应一个[page](https://elixir.bootlin.com/linux/v6.6/source/include/linux/mm_types.h#L74)结构体
